...
class Simple extends AbstractType
{
    ...
    {
        // Eğer kullanıcı login değilse, sepete eklenemez
        if (! auth()->guard('customer')->check()) {
            return false;
        }

        // Eğer ürünün customizable option'ı varsa sepete eklenemez
        if ($this->product->customizable_options->isNotEmpty()) {
            return false;
        }

        // Mevcut kontrolü döndür
    ...
    public function getPriceHtml()
    {
        ...
        if (!$customer) {
            return '<span class="text-danger">Log in to see the price!</span>';
        }
    ...
    /**
     * Sepete ekleme kontrolü (login olmayan kullanıcılar için)
     *
     * @return bool
     */
