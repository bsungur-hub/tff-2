...
class ProductRepository extends Repository
{
    ...
    public function searchFromDatabase(array $params = [])
    {
        ...
        $groupId = $customer ? $customer->customer_group_id : null;
        ...
        $query = $this->with([
        ...
        ])->scopeQuery(function ($query) use ($params, $groupId) {
            ...
            // Kullanıcı grubu ID 4 olmayan kullanıcılara; 1Ana sitede bulunan arama (search) fonksiyonunda 2-Ürün
            // kategorisi ID 2 olan ürünler, Sonuçlarda gösterilmesin


            if ($groupId != 4 && !request()->is('admin/*')) {
                $qb->leftJoin('product_categories as filter_cat', 'products.id', '=', 'filter_cat.product_id')
                    ->where(function ($query) {
                        $query->where('filter_cat.category_id', '!=', 2)
                            ->orWhereNull('filter_cat.category_id');
                    });
            }
