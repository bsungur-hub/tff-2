@component('admin::emails.layout')
    ...
    @php
        $customerGroup = \Webkul\Customer\Models\CustomerGroup::find($order->customer->customer_group_id);

        //customer requested date (customer_note from caart table)

        $cart = \Webkul\Checkout\Models\Cart::where('id', $order->cart_id)->first();
        $customerNote = $cart->customer_note ?? '';

        $date = new \DateTime($customerNote);
    ...
    <div style="margin-bottom: 34px;">
        ...
        <p style="font-size: 16px;color: #0080FE;line-height: 24px;">
    ...
    @if( $customerGroup->name == 'Anatolia')

        <p>{{ $order->shipping_address->company_name }} - {{ $order->shipping_address->name }}
            - {{ $order->shipping_address->address }} - For {{ $formattedDate ?? $formattedDate }}
        </p>

        <p>- You can find the customer's order details in the attached PDF file.</p>
    @else
        ...
        <div
            ...
            @if (core()->getConfigData('sales.taxes.sales.display_subtotal') == 'including_tax')
                <div style="display: grid;gap: 20px;grid-template-columns: repeat(2, minmax(0, 1fr));">
                    <span>@lang('admin::app.emails.orders.subtotal')</span>
                    <span
                        style="text-align: right;">{{ core()->formatBasePrice($order->base_sub_total_incl_tax) }}</span>
                </div>
            @elseif (core()->getConfigData('sales.taxes.sales.display_subtotal') == 'both')
                <div style="display: grid;gap: 20px;grid-template-columns: repeat(2, minmax(0, 1fr));">
                    <span>@lang('admin::app.emails.orders.subtotal-excl-tax')</span>
                    <span style="text-align: right;">{{ core()->formatBasePrice($order->base_sub_total) }}</span>
                </div>
                <div style="display: grid;gap: 20px;grid-template-columns: repeat(2, minmax(0, 1fr));">
                    <span>@lang('admin::app.emails.orders.subtotal-incl-tax')</span>
                    <span
                        style="text-align: right;">{{ core()->formatBasePrice($order->base_sub_total_incl_tax) }}</span>
                </div>
            @else
            ...
            @if ($order->shipping_address)
                @if (core()->getConfigData('sales.taxes.sales.display_shipping_amount') == 'including_tax')
                    <div style="display: grid;gap: 20px;grid-template-columns: repeat(2, minmax(0, 1fr));">
                        <span>@lang('admin::app.emails.orders.shipping-handling')</span>
                        <span
                            style="text-align: right;">{{ core()->formatBasePrice($order->base_shipping_amount_incl_tax) }}</span>
                    </div>
                @elseif (core()->getConfigData('sales.taxes.sales.display_shipping_amount') == 'both')
                    <div style="display: grid;gap: 20px;grid-template-columns: repeat(2, minmax(0, 1fr));">
                        <span>@lang('admin::app.emails.orders.shipping-handling-excl-tax')</span>
                        <span
                            style="text-align: right;">{{ core()->formatBasePrice($order->base_shipping_amount) }}</span>
                    </div>
                    <div style="display: grid;gap: 20px;grid-template-columns: repeat(2, minmax(0, 1fr));">
                        <span>@lang('admin::app.emails.orders.shipping-handling-incl-tax')</span>
                        <span
                            style="text-align: right;">{{ core()->formatBasePrice($order->base_shipping_amount_incl_tax) }}</span>
                    </div>
                @else
            ...
            <div style="display: grid;gap: 100px;grid-template-columns: repeat(2, minmax(0, 1fr));">
                <span>@lang('admin::app.emails.orders.tax')</span>
                <span style="text-align: right;">{{ core()->formatBasePrice($order->base_tax_amount) }}</span>
            </div>

            @if ($order->discount_amount > 0)
                <div style="display: grid;gap: 100px;grid-template-columns: repeat(2, minmax(0, 1fr));">
                    <span>@lang('admin::app.emails.orders.discount')</span>
                    <span style="text-align: right;">{{ core()->formatBasePrice($order->base_discount_amount) }}</span>
                </div>
            @endif

            <div style="display: grid;gap: 100px;grid-template-columns: repeat(2, minmax(0, 1fr));font-weight: bold">
                <span>@lang('admin::app.emails.orders.grand-total')</span>
                <span style="text-align: right;">{{ core()->formatBasePrice($order->base_grand_total) }}</span>
            </div>
        </div>
