...
class Cart
{
    ...
    public function addProduct(ProductContract $product, array $data): Contracts\Cart|\Exception
    {
        ...
        // Guest ve login olup 'guest' kullanıcı grubundakiler (ID:1) kullanıcı sepete ekleyemez
        $isCustomer = auth()->guard()->check();
        $customer = auth()->guard('customer')->user();
        if (!$isCustomer || $customer->customer_group_id == 1 ) {
            throw new \Exception('Log in to see the price and add to cart!');
