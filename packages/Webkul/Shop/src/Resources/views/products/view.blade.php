{{--Urun Kategori ve Kullanıcı grubu kontrolleri--}}

@php
    $customer = auth()->guard('customer')->user();
    $anatoliaCategoryId = 2; // Kontrol edilecek kategori ID
    $allowedGroupId = 4;     // Erişebilecek kullanıcı grubu ID

    // Ürün Anatolia kategorisinde mi?
    $isAnatoliaProduct = $product->categories->pluck('id')->contains($anatoliaCategoryId);

    // Eğer ürün kategori 2 ise ve kullanıcı grubu 4 değilse engelle
    if ($isAnatoliaProduct && (! $customer || $customer->customer_group_id !== $allowedGroupId)) {
        abort(404, 'You are not allowed to view this product.');
    }
@endphp
                                ...
                                <div class="mt-8 flex max-w-[470px] gap-4 max-sm:mt-4">
                                    ...
                                    @if (core()->getConfigData('sales.checkout.shopping_cart.cart_page'))
                                        ...
                                        <x-shop::button
                                            ...
                                            :disabled="! auth()->guard('customer')->check() || ! $product->isSaleable(1)"
