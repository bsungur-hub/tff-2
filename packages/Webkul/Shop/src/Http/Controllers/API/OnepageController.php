...
class OnepageController extends APIController
{
    ...
    public function storeAddress(CartAddressRequest $cartAddressRequest): JsonResource
    {
        ...
        $cartAddressRequest->validate([
            'customer_note' => 'required|date',
        ]);
        ...
        try {
            ...
            if ($cart) {
                $cart->customer_note = $cartAddressRequest->input('customer_note');
                $cart->save();
            }
        } catch (\Exception $e) {
            \Log::error('Failed to save customer_note: ' . $e->getMessage());
        }
