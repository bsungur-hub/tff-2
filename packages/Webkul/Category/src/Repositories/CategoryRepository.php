...
class CategoryRepository extends Repository
{
    ...
    public function getVisibleCategoryTree($id = null)
    {
        ...
        // 2️⃣ Eğer admin paneli değilsek, guest veya grup ID!=4 için filtre uygula
        if (!Request::is('admin/*')) { // frontend kontrolü
            $user = Auth::guard('customer')->user();

            if (!$user || $user->customer_group_id != 4) {
                // Top-level kategorilerde filtre
                $tree = $tree->filter(function ($category) {
                    return $category->id != 2;
                });

                // Eğer alt kategoriler varsa, recursive filtreleme
                $tree->each(function ($category) {
